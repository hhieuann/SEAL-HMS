...
class RankingControllerTest {
    ...
    void computeRoundRanking_asAdmin_ok() throws Exception {
        when(rankingService.computeRoundRanking(eq(1L), any())).thenReturn(List.<RoundStandingDto>of());

        mockMvc.perform(post("/api/v1/rounds/1/ranking/compute")
                        .with(user("admin").roles("ADMIN")).with(csrf())
                        .contentType("application/json").content("[]"))
                .andExpect(status().isOk());
        verify(rankingService).computeRoundRanking(eq(1L), any());
    }
    ...
    void computeRoundRanking_asStudent_forbidden() throws Exception {
        ...
        verify(rankingService, never()).computeRoundRanking(any(), any());
    }
